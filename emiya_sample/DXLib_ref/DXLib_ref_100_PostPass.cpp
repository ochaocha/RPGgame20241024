...
namespace DXLibRef {
	// シングルトンの実態定義
	const PostPass::PostPassEffect* SingletonBase<PostPass::PostPassEffect>::m_Singleton = nullptr;
	namespace PostPass {
		// --------------------------------------------------------------------------------------------------
		// ポストプロセス継承クラス
		// --------------------------------------------------------------------------------------------------
		...
		public:
			void Load_Sub(void) noexcept override {
				...
				GaussScreen_.Make(WindowSizeParts->GetSizeXMax() / EXTEND, WindowSizeParts->GetSizeYMax() / EXTEND, true);
				BufScreen.Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), true);
			}
			...
			void SetEffect_Sub(GraphHandle* TargetGraph, GraphHandle*) noexcept override {
				...
				TargetGraph->SetDraw_Screen(false);
				{
					...
					GaussScreen_.DrawExtendGraph(0, 0, WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), true);
					GaussScreen_.DrawExtendGraph(0, 0, WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), true);
		...
		public:
			void Load_Sub(void) noexcept override {
				...
				for (auto& buf : BufScreen) {
					buf.Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), true);
				}
			}
			void Dispose_Sub(void) noexcept override {
				for (auto& buf : BufScreen) {
			...
			void SetEffect_Sub(GraphHandle* TargetGraph, GraphHandle*) noexcept override {
				...
				TargetGraph->SetDraw_Screen(false);
				{
					...
					SetDrawBlendMode(DX_BLENDMODE_ADD, 255);
					BufScreen[0].DrawRotaGraph(WindowSizeParts->GetSizeXMax() / 2, WindowSizeParts->GetSizeYMax() / 2, 1.f + 0.005f * PostPassParts->GetAberrationPower(), 0.f, true);
					BufScreen[1].DrawRotaGraph(WindowSizeParts->GetSizeXMax() / 2, WindowSizeParts->GetSizeYMax() / 2, 1.f, 0.f, true);
					BufScreen[2].DrawRotaGraph(WindowSizeParts->GetSizeXMax() / 2, WindowSizeParts->GetSizeYMax() / 2, 1.f - 0.005f * PostPassParts->GetAberrationPower(), 0.f, true);
					SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 255);
				}
			}
		};
		...
		private:
			...
			public:
				...
				~BlurScreen(void) noexcept {}
			public:
				void Initialize(int t_alpha, int t_offsetX1, int t_offsetY1, int t_offsetX2, int t_offsetY2) noexcept {
					auto* WindowSizeParts = WindowSizeControl::Instance();
					for (int i = 0; i < MAX; ++i) {
						m_screen.at(static_cast<size_t>(i)).Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax());
					}
					m_current = 0;
					m_alpha = t_alpha;
				...
				auto* PostRenderBlurScreen(std::function<void()> doing) noexcept {
					...
					m_screen[static_cast<size_t>(m_current)].SetDraw_Screen();
					{
						...
						if (++m_notBlendDraw > MAX) {
							...
							m_screen[static_cast<size_t>(next)].DrawExtendGraph(m_offsetX1, m_offsetY1, WindowSizeParts->GetSizeXMax() + m_offsetX2, WindowSizeParts->GetSizeYMax() + offsetY2, false);
							SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 255);
		...
		public:
			void Load_Sub(void) noexcept override {
				m_BlurScreen.Initialize(96, -6, -6, 6, 6);
			}
			void Dispose_Sub(void) noexcept override {
		...
		public:
			void Load_Sub(void) noexcept override {
				auto* WindowSizeParts = WindowSizeControl::Instance();
				AberrationScreen.Make(WindowSizeParts->GetSizeXMax() / EXTEND, WindowSizeParts->GetSizeYMax() / EXTEND, true);
				{
					bkScreen2.Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), false);
					bkScreen2.SetDraw_Screen(false);
					{
						bkScreen2.FillGraph(0, 0, 0);
						int xr = WindowSizeParts->GetSizeXMax() * 60 / 100;
						int yr = WindowSizeParts->GetSizeYMax() * 70 / 100;

						DxLib::DrawOval(WindowSizeParts->GetSizeXMax() / 2, WindowSizeParts->GetSizeYMax() / 2, xr, yr, White, TRUE);
						...
						for (r = 0; r < 255; r += p) {
							...
							DxLib::DrawOval(WindowSizeParts->GetSizeXMax() / 2, WindowSizeParts->GetSizeYMax() / 2, xr - r / p, yr - r / p, GetColor(c, c, c), FALSE, 2);
						}
					}
				}
				BufScreen.Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), true);
			...
			void SetEffect_Sub(GraphHandle* TargetGraph, GraphHandle*) noexcept override {
				...
				TargetGraph->SetDraw_Screen(false);
				{
					AberrationScreen.DrawExtendGraph(0, 0, WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), false);
		...
		public:
			void Load_Sub(void) noexcept override {
				auto* WindowSizeParts = WindowSizeControl::Instance();
				{
					bkScreen.Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), false);
					...
					bkScreen.FillGraph(255, 255, 255);
					{
						...
						for (y = 0; y < 255; y += p) {
							...
							DxLib::DrawLine(0, y / p, WindowSizeParts->GetSizeXMax(), y / p, GetColor(c, c, c));
						}
						p = 2;
						for (y = 0; y < 255; y += p) {
							c = 255 - int(std::powf(float(255 - y) / 255.f, 1.5f) * 128.f);
							DxLib::DrawLine(0, WindowSizeParts->GetSizeYMax() - y / p, WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax() - y / p, GetColor(c, c, c));
						}
					}
				}
				BufScreen.Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), true);
			...
			void DrawCircleScreen(
				...
				for (i = 0; i < CIRCLE_ANGLE_VERTEX_NUM; ++i) {
					...
					for (int j = 0; j < CIRCLE_RADIUS_VERTEX_NUM; ++j, ++Vert) {
						...
						// テクスチャ座標のセット
						Vert->u = (Vert->pos.x + AbsorptionMoveX) / static_cast<float>(WindowSizeParts->GetSizeXMax());
						Vert->v = (Vert->pos.y + AbsorptionMoveY) / static_cast<float>(WindowSizeParts->GetSizeYMax());
				...
				for (i = 0; i < CIRCLE_ANGLE_VERTEX_NUM; ++i) {
					...
					for (int j = 0; j < CIRCLE_RADIUS_VERTEX_NUM; ++j, ++Vert) {
						...
						// テクスチャ座標のセット
						Vert->u = (Cos * GraphCenterDistance + static_cast<float>(CenterX)) / static_cast<float>(WindowSizeParts->GetSizeXMax());
						Vert->v = (Sin * GraphCenterDistance + static_cast<float>(CenterY)) / static_cast<float>(WindowSizeParts->GetSizeYMax());
		...
		public:
			...
			void SetEffect_Sub(GraphHandle* TargetGraph, GraphHandle*) noexcept override {
				...
				TargetGraph->SetDraw_Screen();
				{
					// 画面を歪ませて描画
					DrawCircleScreen(
						WindowSizeParts->GetSizeXMax() / 2, WindowSizeParts->GetSizeYMax() / 2,
						static_cast<float>(WindowSizeParts->GetSizeXMax() * 2 / 3), 120.0f, BufScreen);
				}
			}
		};
		// --------------------------------------------------------------------------------------------------
		...
		PostPassEffect::PostPassEffect(void) noexcept {
			...
			BufferScreen.Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), true);
			ColorScreen.Make(WindowSizeParts->GetSizeXMax(), WindowSizeParts->GetSizeYMax(), false);
